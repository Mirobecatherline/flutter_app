...
class ReceivePackagePage extends StatefulWidget {
  const ReceivePackagePage({Key? key}) : super(key: key);

  @override
  State<ReceivePackagePage> createState() => _ReceivePackagePageState();
}


class _ReceivePackagePageState extends State<ReceivePackagePage> {
                      ...
                      child: Container(
                        ...
                        child: TextField(
                          ...
                          decoration:InputDecoration(
                            ...
                            hintText: "Instructions ( Optional ) e.g Room no",
                            hintStyle:  const TextStyle(color: Colors.black38,fontWeight: FontWeight.normal,fontSize: 14),
                          ...
                          onChanged: (text) {
                           if (text.length > 65) {
                              instructionsTextEditingController.text = text.substring(0, 65);
                            ...
                            Checkbox(
                              ...
                              onChanged: (newValue) {
                                setState(() {
                                  ...
                                  deliveryPaymentDoneBy = isCheckBoxChecked ? "Sender Pays" : "Sender Pays";
  ...
  void _onSearchTextChanged(String query) async {
    ...
    final String apiUrl = 'https://autocomplete.search.hereapi.com/v1/autosuggest?at=-1.2921,36.8219&limit=8&q=$query&apiKey=$apiKey&country=KEN';
   try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          searchSuggestionsTitle = List<String>.from(suggestions.map((item) => item['title'])); // Extract titles
          ...
          searchSuggestionsRoad = List<String>.from(suggestions.map((item) {
            ...
            List<String> addressParts = label.split(', '); // Split the label by comma and space
          ...
          searchSuggestionsLatitude = List<double>.from(suggestions.map((item) => item['position']['lat']));
          searchSuggestionsLongitude = List<double>.from(suggestions.map((item) => item['position']['lng']));
  ...
  void _onSearchTextChanged(String query) async {
    ...
    String apiKey = hereMapsKey; // Replace with your HERE Maps API key
    final String apiUrl =
        'https://autocomplete.search.hereapi.com/v1/autosuggest?at=-1.2921,36.8219&limit=8&q=$query&apiKey=$apiKey&country=KEN';
    try {
      var response = await http.get(Uri.parse(apiUrl));
      if (response.statusCode == 200) {
        Map<String, dynamic> data = json.decode(response.body);
...
void main() {
  runApp(const MaterialApp(home: ReceivePackagePage()));
}